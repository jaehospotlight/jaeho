...
class ReadingTests(unittest.TestCase):
    def test_existing_instinct_summary_is_not_a_personal_note(self):
    ...
    def test_edit_is_reflected_on_next_build(self):
        entry, = reading_entries([issue(title="Updated title", body="https://x.com/a/status/123\nPersonal note: Updated note")], "jaehospotlight")
        self.assertEqual(entry["title"], "Updated title")
        self.assertEqual(entry["note"], "Updated note")

    def test_no_personal_note_means_no_blurb(self):
        for content in ("", "\nAn unmarked generated blurb.", "\nNote: A generic AI note.", "\n### Summary\nA generated summary.", "\nPersonal note:", "\n### Personal note\n_No response_"):
            with self.subTest(content=content):
                entry, = reading_entries([issue(body="https://x.com/a/status/123" + content)], "jaehospotlight")
                self.assertEqual(entry["note"], "")

    def test_personal_note_keeps_wording_and_excludes_surrounding_summary(self):
        body = "https://x.com/a/status/123\nSummary: AI summary.\n\nPersonal note: My exact words.\nAnother line.\n\nA second paragraph.\n\nSummary: Another AI summary."
        entry, = reading_entries([issue(body=body)], "jaehospotlight")
        self.assertEqual(entry["note"], "My exact words.\nAnother line.\n\nA second paragraph.")
